...
if (!$modification_type || $amount == 0) {
    $_SESSION['nin_update_error'] = "Invalid modification type.";
    header('Location: update_nin.php');
    exit();
}
...
if (isset($_FILES['support_doc']) && $_FILES['support_doc']['error'] == UPLOAD_ERR_OK) {
    ...
    $target_dir = "../uploads/nin_docs/";
    if (!is_dir($target_dir)) mkdir($target_dir, 0777, true);
    $filename = "nin_" . $user_id . "_" . time() . "." . $ext;
    $target_file = $target_dir . $filename;
    if (move_uploaded_file($_FILES['support_doc']['tmp_name'], $target_file)) {
        $support_doc_path = "uploads/nin_docs/" . $filename;
    }
}
...
// Insert into nin_update_requests table
$stmt = $conn->prepare("INSERT INTO nin_update_requests 
...
$reference = 'NIN-' . $user_id . '-' . time() . '-' . bin2hex(random_bytes(4));
$description = "NIN Update ($modification_type)";
...
$_SESSION['nin_update_success'] = "Your NIN update request has been submitted. Our team will process it shortly.";
header('Location: update_nin.php?success=1');
exit();
